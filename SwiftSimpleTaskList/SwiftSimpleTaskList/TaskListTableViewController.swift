    ...
    */
    ...
    private func loadTaskData() {
        ...
        do {
            ...
            self.showAlertMessage(alertTitle: "Data Fetch Error(throw)", alertMessage: error.localizedDescription)
        }        
    ...
    private func deleteTaskRecord(indexPath: NSIndexPath){
        ...
        do {
            ...
            // 4 - delete record from task list array
            self.taskList.removeAtIndex(indexPath.row)
            
            // 5 - delete table row with animation
            self.tableView.deleteRowsAtIndexPaths([indexPath], withRowAnimation: UITableViewRowAnimation.Fade)
            
            // 6 - if no record then toggle table editing mode
            if self.tableView.editing &&  self.taskList.count == 0 {
                self.toggleTableEditingMode()
            }
            
            // 7 - disable edit button if no record
            self.enableDisableEditButton()
