...
class TaskEditViewController: UIViewController, UITextFieldDelegate, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    private func saveRecord(){
        ...
        do {
            ...
            // Step 4:  call task update delegate function
            self.taskDelegate?.taskDidUpdate(self.selectedTask!)
            
            // Step 5:  Go to back screen if no error
            self.goBack()
            
        } catch let error as NSError {
            self.showAlertMessage(alertTitle: "Update Error", alertMessage: error.localizedDescription  )
        }
    ...
    private func getIndexForColor(colorHexValue colorHexValue: String) -> Int {
        var selectedIndex = 0
