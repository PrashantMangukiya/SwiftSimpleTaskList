...
class TaskAddViewController: UIViewController, UITextFieldDelegate, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    private func saveRecord(){
        ...
        // Step 5: Save data into database
        var error: NSError?
        managedObjectContext.save(&error)
        
        // Step 6: Show error if generated during save operation
        if let err = error {
            // show custom error message to user
            self.showAlertMessage(alertTitle: "Save Error", alertMessage: "Error while save, please try again.")
        }else{
            
            // call task delegate function
